...
import pandas as pd
import matplotlib
matplotlib.use('Agg')  # 使用非交互式后端，避免 GUI 线程错误
...
def calculate_and_plot_stats(df, district_name):
    ...
    plt.plot(average_prices.index, average_prices, marker='o', linestyle='-', label='average')
    plt.plot(median_prices.index, median_prices, marker='x', linestyle='--', label='middle')
    plt.xticks(rotation=45)
    plt.ylabel('price')
    plt.title(f'AERA：{district_name} - ')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    return average_prices, median_prices
